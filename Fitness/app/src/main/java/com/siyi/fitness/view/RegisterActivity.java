...
import com.siyi.fitness.utils.AppManager;
...
public class RegisterActivity extends BaseActivity implements OnClickListener {
    private String TITLE_NAME = "Register";
    ...
    protected void initView() {
        ...
        uiFlusHandler = new MyDialogHandler(mContext, "Registering...");
    ...
    private void register() {
        ...
        // 判断用户名密码是否为空，不为空请求服务器（省略，默认请求成功）i
        if (TextUtils.isEmpty(username) || TextUtils.isEmpty(password) || TextUtils.isEmpty(repassword) || TextUtils.isEmpty(height) || TextUtils.isEmpty(weight)) {
            DisplayToast("Information cannot be empty");
            return;
        }
        // 判断两次密码
        if (!password.equals(repassword)) {
            DisplayToast("Two password entries are inconsistent");
            return;
        }
            ...
            switch (id) {
                case 1:
                    ...
                    if (user == null) {
                        ...
                        if (result) {
                            Toast.makeText(mContext, "Registered successfully, please log in", Toast.LENGTH_SHORT).show();
                        } else {
                            Toast.makeText(mContext, "Failed to save username and password", Toast.LENGTH_SHORT).show();
                        }
                    }
